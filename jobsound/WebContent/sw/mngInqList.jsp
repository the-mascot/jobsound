         ...
         function() {
            $("#viewCount").change(
                  function() {
                     ...
                     location.href = "<%=context%>/sw/mngInqList.do?pageSize=" + pageSize
                           + "&pageNum=" + pageNum;
                  });
         });
   function search(b_type) {
      var search = $("#search").val();
      location.href="<%=context%>/sw/mngInqList.do?search="+search+"&b_type="+b_type;
   }
</script>
		...
		<c:if test="${totCnt > 0}">
			<c:forEach var="Inquire" items="${list}">
				<tr>
					...
					<td><a href='<%=context%>/sw/mngInq.do?inqnum=${Inquire.inqnum}' style="text-decoration:none; color:black;">${Inquire.inqtitle}</a></td>
					<td>${Inquire.id}</td>
         ...
         <nav aria-label="Page navigation example">
               ...
               <c:choose>
                  <c:when test="${startPage>blockSize }">
                     <li class="page-item"><a class="pagenav_a page-link"
                        href="mngInqList.do?pageNum=${startPage-blockSize }&pageSize=${pageSize }"
            ...
            <!-- 이전 페이지 -->
               <c:choose>
                  ...
                  <c:otherwise>
                     <li class="page-item"><a class="pagenav_a page-link"
                        href="mngInqList.do?pageNum=${pageNum-1 }" aria-label="Previous">
                           <span aria-hidden="true">&lt;</span>
                     </a></li>
                  </c:otherwise>
               </c:choose>
            <!-- 페이지 목록 -->
               <c:forEach var="i" begin="${startPage }" end="${endPage }">
                  <li class="page-item"><a class="pagenav_a page-link"
                     href="mngInqList.do?pageNum=${i }&pageSize=${pageSize }">[${i }]</a></li>
               </c:forEach>
            <!-- 다음 페이지 -->
               <c:choose>
            ...
            <!-- 다음 10개 페이지 -->
               <c:choose>
                  <c:when test="${endPage<pageCnt }">
                     <li class="page-item"><a class="pagenav_a page-link"
                        href="mngInqList.do?pageNum=${startPage+blockSize }"
