         ...
         function() {
            $("#viewCount").change(
                  function() {
                     ...
                     location.href = "<%=context%>/sw/mngCommList.do?pageSize=" + pageSize
                           + "&pageNum=" + pageNum;
                  });
         });
   function search(b_type) {
      var search = $("#search").val();
      location.href="<%=context%>/sw/mngCommList.do?search="+search+"&b_type="+b_type;
   }
</script>
         ...
         <nav aria-label="Page navigation example">
               ...
               <c:choose>
                  <c:when test="${startPage>blockSize }">
                     <li class="page-item"><a class="pagenav_a page-link"
                        href="mngCommList.do?pageNum=${startPage-blockSize }&pageSize=${pageSize }"
            ...
            <!-- 이전 페이지 -->
               <c:choose>
                  ...
                  <c:otherwise>
                     <li class="page-item"><a class="pagenav_a page-link"
                        href="mngCommList.do?pageNum=${pageNum-1 }" aria-label="Previous">
                           <span aria-hidden="true">&lt;</span>
                     </a></li>
                  </c:otherwise>
               </c:choose>
            <!-- 페이지 목록 -->
               <c:forEach var="i" begin="${startPage }" end="${endPage }">
                  <li class="page-item"><a class="pagenav_a page-link"
                     href="mngCommList.do?pageNum=${i }&pageSize=${pageSize }">[${i }]</a></li>
               </c:forEach>
            <!-- 다음 페이지 -->
               <c:choose>
            ...
            <!-- 다음 10개 페이지 -->
               <c:choose>
                  <c:when test="${endPage<pageCnt }">
                     <li class="page-item"><a class="pagenav_a page-link"
                        href="mngCommList.do?pageNum=${startPage+blockSize }"
