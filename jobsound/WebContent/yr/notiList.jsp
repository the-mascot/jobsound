			...
			function() {
				$("#viewCount").change(
						function() {
							...
							location.href = "<%=context%>/yr/notiList.do?pageSize=" + pageSize
									+ "&pageNum=" + pageNum;
						});
			});
				...
				<div class="board_search d-flex">
					<div>
						<input type="text" class="form-control" placeholder="Search"
							maxlength="255" name="search" id="search">
					</div>
					<div>
						...
						<c:forEach var="board" items="${list }" varStatus="status">
							<tr>
								...
								<td>
									 ...
									 <a href='notiContent.do?b_num=${board.b_num }&pageNum=${currentPage}' style="text-decoration:none; color:black; float: left" >
											${board.title }</a>
									<c:if test="${today == board.uploadDate }">
											<img src='<%=context %>/common/images/Newicon2.png' width="30px" height="10px" style="float: left; margin-left: 10px">
									</c:if>
								</td>
								<td>${board.id }</td>
			...
			<nav aria-label="Page navigation example">
					...
					<c:choose>
						<c:when test="${startPage>blockSize }">
							<li class="page-item"><a class="pagenav_a page-link"
								href="notiList.do?pageNum=${startPage-blockSize }&pageSize=${pageSize }"
				...
				<!-- 이전 페이지 -->
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="pagenav_a page-link"
								href="notiList.do?pageNum=${pageNum-1 }" aria-label="Previous">
									<span aria-hidden="true">&lt;</span>
							</a></li>
						</c:otherwise>
					</c:choose>
				<!-- 페이지 목록 -->
					<c:forEach var="i" begin="${startPage }" end="${endPage }">
						<li class="page-item"><a class="pagenav_a page-link"
							href="notiList.do?pageNum=${i }&pageSize=${pageSize }">[${i }]</a></li>
					</c:forEach>
				<!-- 다음 페이지 -->
					<c:choose>
				...
				<!-- 다음 10개 페이지 -->
					<c:choose>
						<c:when test="${endPage<pageCnt }">
							<li class="page-item"><a class="pagenav_a page-link"
								href="notiList.do?pageNum=${startPage+blockSize }"
