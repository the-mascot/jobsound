...
<script type="text/javascript">
	...
	$(document).ready(function(){		
		...
		$("#stdstart_date, #stdend_date").on("change", function() {
	        var stdStartDate=$("#stdstart_date").val();
	        var endDate=$("#stdend_date").val();
	        var recruStartDate=$("#recru_date").val();
	        
	        var result1="※종료일이 시작일 보다 빠릅니다";
	        var result2="※시작일을 오늘 이후로 설정 하십시오";
	        var result3="※모집 시작일은 스터디 시작일보다 빨라야 합니다"
			today=getToday();
	   
	      	if((stdStartDate>endDate) && (endDate!="")) {
		        $('#checkDate').html(result1);
				$("#stdend_date").val("");
			} else if(stdStartDate<today && (stdStartDate!="")) {
				$('#checkDate').html(result2);
				$("#stdstart_date").val("");				
			} else if (recruStartDate>stdStartDate) {
				$('#checkDate').html(result3);
				$("#stdstart_date").val("");
			} else {
				$('#checkDate').html("");
			}
		...
		$("#recru_date, #recu_date").on("change", function() {
			var stdStartDate=$("#stdstart_date").val();
	        var recuStartDate=$("#recru_date").val();
			...
			console.log(today)
	        console.log(endDate)
	      	if((recuStartDate>endDate) && (endDate!="")) {
		        $('#checkDate2').html(result1);
				$("#recu_date").val("");
			} else if(recuStartDate<today) {
				$('#checkDate2').html(result2);
				$("#recru_date").val("");				
			} else if (recuStartDate>stdStartDate && (stdStartDate!="")) {
				$('#checkDate2').html(result3);
				$("#recru_date").val("");
			} else {
				$('#checkDate2').html("");
			}
...
<section>
	...
	<div class="container">
		<div class="col-md-10 mx-auto my-1" style="min-height: 700px;">
			<form action="<%=context %>/es/recuUpdatePro.do" method="post">
				<input type="hidden" value="${pageNum }" name="pageNum">
				<input type="hidden" value="${pageSize }" name="pageSize">
				<input type="hidden" value="${b_num }" name="b_num">
				<input type="hidden" value="${board.stdnum }" name="stdnum">				<!-- 등록 / 취소 버튼 -->
				...
				<div class="writeFormTitle col-12 mt-5 mb-5" style="clear: both;">
					...
					<div>
						<input type="text" class="form-control" id="title" name="title"
							placeholder="글 제목을 입력하세요" value="${board.title }" required="required">	
					</div>
				</div>
				<!-- 스터디 정보 입력 -->
				<div style="border: 1px solid rgba(65, 75, 70, 0.2);">
					<div class="col-8 my-3 ms-3">
						...
						<input type="text" class="form-control" id="stdtitle" name="stdtitle"
							placeholder="스터디 제목을 입력하세요" value="${board.stdtitle }" required="required">
					</div>
					<div class="col-3 my-3 ms-3">
						...
						<input type="text" class="form-control" id="stddiv" name="stddiv"
							placeholder="스터디 분류를 입력하세요" value="${board.stddiv }" required="required" maxlength="10">
					</div>
						...
						<div class="row">
							<div class="col-2">
								<div>
									...
									<input type="date" id="stdstart_date" name="stdstart_date" value="${board.stdstart_date }" required="required">
							</div>
							<div class="col-2 ms-5">
								<div>
									<label for="stdend_date">스터디 마감일</label>
								</div>
									<input type="date" id="stdend_date" name="stdend_date" value="${board.stdend_date }" required="required">
							</div>
						...
						<div class="row">
							<div class="col-2">
								<div>
									...
									<input type="date" id="recru_date" name="recru_date" value="${board.recru_date }" readonly="readonly" required="required">
							</div>
							<div class="col-2 ms-5">
								<div>
									<label for="recu_date">모집 마감일</label>
								</div>
									<input type="date" id="recu_date" name="recu_date" value="${board.recu_date }" required="required">
							</div>
							<div class="col ms-4 mt-4">
				...
				<div class="mt-3">
					...
					<div>
						<textarea class="form-control" id="content" name="content" 
							placeholder="글 내용을 입력하세요" style="min-height: 300px;"><c:out value="${board.content }"></c:out></textarea>	
					</div>
				</div>
