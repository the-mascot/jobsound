  		...
  		<c:if test="${totCnt>0}">
	  		<c:forEach var = "board" items="${list }" varStatus="status">
			  	<tr>
			  		...
			  		<td colspan="2"><a href="../es/recuContent.do?pageNum=${pageNum}&b_num=${board.b_num}">${board.title }</a></td><!-- 경로이동 해야됨  -->
        ...
        <ul class="pagination justify-content-center mt-5">
            ...
            <c:choose>
                <c:when test="${startPage>blockSize }">
                    <li class="page-item">
                        <a class="pagenav_a page-link" href="<%=context %>/di/mypageStudyList.do?b_type=2&pageNum=${startPage-blockSize }" aria-label="Previous"> 
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <li class="page-item">
                        <a class="pagenav_a page-link" href="<%=context %>/di/mypageStudyList.do?b_type=2&pageNum=${pageNum-1 }" aria-label="Previous">
                        <span aria-hidden="true">&lt;</span></a>
                    </li>
                </c:otherwise>
            </c:choose>
            ...
            <c:forEach var="i" begin="${startPage }" end="${endPage }">
                <li class="page-item">
                    <a class="pagenav_a page-link" href="<%=context %>/di/mypageStudyList.do?b_type=2&pageNum=${i }">[${i }]</a></li>
            </c:forEach>
            <!-- 다음 페이지 -->
            <c:choose>
            ...
            <c:choose>
                <c:when test="${endPage<pageCnt }">
                    <li class="page-item">
                        <a class="pagenav_a page-link" href="<%=context %>/di/mypageStudyList.do?b_type=2&pageNum=${startPage+blockSize }" aria-label="Next">
