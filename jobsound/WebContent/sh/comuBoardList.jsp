...
<c:otherwise>
<title>커뮤니티 게시판 - ${keyWord } 검색결과</title>
</c:otherwise>
</c:choose>
<!-- 부트스트랩 CDN -->
<!-- 부트스트랩 CDN -->
...
<section>
	...
	<div class="board_search_parent">
		<form action="<%=context%>/sh/comuSearch.do?pageSize=${pageSize }" id="frm" onsubmit="return chk()">
				...
				<c:if test="${totCnt>0}">
					<c:forEach var="board" items="${list}">
						<tr>
							...
							<a href='<%=context%>/sh/comuContent.do?b_num=${board.b_num}&pageNum=${currentPage}&pageSize=${pageSize}&b_type=${b_type}' style="text-decoration: none; color: black;" class="title">
									${board.title} </a>
  						...
  						<path d="M14.002 13a2 2 0 0 1-2 2h-10a2 2 0 0 1-2-2V5A2 2 0 0 1 2 3a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v8a2 2 0 0 1-1.998 2zM14 2H4a1 1 0 0 0-1 1h9.002a2 2 0 0 1 2 2v7A1 1 0 0 0 15 11V3a1 1 0 0 0-1-1zM2.002 4a1 1 0 0 0-1 1v8l2.646-2.354a.5.5 0 0 1 .63-.062l2.66 1.773 3.71-3.71a.5.5 0 0 1 .577-.094l1.777 1.947V5a1 1 0 0 0-1-1h-10z"/>
							...
							<img src="<%=context %>/common/images/new.png">
					</c:if></td>
	...
	<div class="write_btn_parent">
		<div class="write_btn">
			<a href="<%=context%>/sh/comuWriteForm.do" class="write_btn btn btn-outline-primary"
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination justify-content-center mt-5">
			...
			<c:choose>
				...
				<c:otherwise>
					<li class="page-item">
						<a class="pagenav_a page-link" href="<%=context%>/sh/comuBoardList.do?pageNum=${pageNum-1 }&keyWord=${keyWord}" aria-label="Previous">
						<span aria-hidden="true">&lt;</span></a>
					</li>
				</c:otherwise>
			</c:choose>
			...
			<c:forEach var="i" begin="${startPage }" end="${endPage }">
				<li class="page-item">
					<a class="pagenav_a page-link" id="pageNav${i}" href="<%=context%>/sh/comuBoardList.do?pageNum=${i }&pageSize=${pageSize }&keyWord=${keyWord}">[${i }]</a></li>
			</c:forEach>
			<!-- 다음 페이지 -->
			<c:choose>
			...
			<c:choose>
				<c:when test="${endPage<pageCnt }">
					<li class="page-item">
						<a class="pagenav_a page-link" href="<%=context%>/sh/comuBoardList.do?pageNum=${startPage+blockSize }&keyWord=${keyWord}" aria-label="Next">
